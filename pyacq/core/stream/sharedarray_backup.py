...
import numpy as np
import sys, random, string, tempfile, mmap, os, stat

# TODO
# On POSIX system it can optionally the shm_open way to avoid mmap.

# RD: use linux style mmap even on windows
unix_style_mmap = False
...
class SharedMem:
    ...
    def __init__(self, nbytes, shm_id=None):
        ...
        if sys.platform.startswith('win'):
            if unix_style_mmap:
                # mmap to file
                if shm_id is None:
                    self._tmpFile = tempfile.NamedTemporaryFile(dir='./', prefix=u'pyacq_SharedMem_')
                    ...
                    self.mmap = mmap.mmap(self._tmpFile.fileno(), self.nbytes, access=mmap.ACCESS_WRITE)
                    os.chmod(self.shm_id, 0o777)
                    print('class SharedMem: writing; pid = {}; self.shm_id = {}'.format(self.pid, self.shm_id))
                else:
                    self._tmpFile = open(self.shm_id, 'rb')
                    print('class SharedMem: reading; pid = {}; self.shm_id = {}'.format(self.pid, self.shm_id))
                    self.mmap = mmap.mmap(self._tmpFile.fileno(), self.nbytes, access=mmap.ACCESS_READ)
            else:
                # windows style mmap
                if shm_id is None:
    ...
    def close(self):
        ...
        """
        self.mmap.close()
        if ((not sys.platform.startswith('win')) or unix_style_mmap) and hasattr(self, '_tmpFile'):
            print('class SharedMem: closing _tmpFile')
            self._tmpFile.close()
